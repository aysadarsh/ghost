...
const handleSubmit = async (e: Event) => {
  try {
    ...
    const get = await HandleApi(`licence/create`, {
      method: 'POST',
    ...
    if (pass(get)) {
      Swal.fire({
        title: 'Success!',
        text: 'Licence created successfully',
        icon: 'success'
      })

      return router.push({ name: 'admin-licence' })
    } else {
      if (response.errors) {
        ...
        <div className="card h-full">
          <div className="card-body">
            <div class="flex justify-between">
              <div>
                <h4 className="text-gray-600 text-lg font-semibold mb-6">Create Licence</h4>
              </div>
              <div>
                <RouterLink
                  type="submit"
