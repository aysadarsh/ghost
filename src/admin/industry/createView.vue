...
const formSchema = z.object({
  name: z.string().min(1, 'Please enter industry name')
})
...
const handleSubmit = async (e: Event) => {
  try {
    ...
    const get = await HandleApi(`industry/create`, {
      method: 'POST',
    ...
    if (pass(get)) {
      ...
      return router.push({ name: 'admin-industry' })
    } else {
      if (response.errors) {
        ...
        <div className="card h-full">
          <div className="card-body">
            <div class="flex justify-between">
              <div>
                <h4 className="text-gray-600 text-lg font-semibold mb-6">Create Industry</h4>
              </div>
              <div>
                <RouterLink
                  type="submit"
