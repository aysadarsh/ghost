...
const validSchema = require('../../schemas/licenceSchema')
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const get = await Model.findById(id)
    if (!get) {
      return res.status(400).send({ message: 'Invalid licence' })
    }

    // check if same name already exists
    // const alreadyExists = await Model.findOne({ name: validatedData.data.name })
    // if (alreadyExists) {
    //   return res.status(400).send({ message: 'licence already exists' })
    // }
...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    if (alreadyExists) {
      return res.status(400).json({ message: 'Licence name already exists!' })
    }
